    ...
    try {
      rdd.foreachPartition(partition => {
        ...
        partition.foreach(record => {
          ...
          val findBasic = new BasicDBObject(Common.MONGO_HISTORY_SHOP_DAYS, new BasicDBObject(Common.MONGO_OPTION_SLICE, List[Int](-1, 1))).append(Common.MONGO_HISTORY_SHOP_FIRSTDATE, 1)

          val updateBasic = new BasicDBObject()
            .append(Common.MONGO_OPTION_INC, new BasicDBObject(Common.MONGO_HISTORY_SHOP_TIMES, 1))
            .append(Common.MONGO_OPTION_SET, new BasicDBObject(Common.MONGO_HISTORY_SHOP_LASTDATE, dayTime))
          ...
          if (retList.size > 0) {
            val record = retList(0)
            if (! record.containsField(Common.MONGO_HISTORY_SHOP_FIRSTDATE)) {
              val firstDayCol = new BasicDBObject(Common.MONGO_HISTORY_SHOP_DAYS, new BasicDBObject(Common.MONGO_OPTION_SLICE, 1))
...
//              val firstDay = historyCollection.find(queryBasic, firstDayCol).toList
//              if (firstDay.size > 0) {
//                val firstTime =  firstDay(0).get(Common.MONGO_HISTORY_SHOP_DAYS).asInstanceOf[List[Long]]
//                dayTime = firstTime(0)
